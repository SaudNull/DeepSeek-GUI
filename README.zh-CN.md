...
> 一个 DeepSeek-first 的桌面 AI 工作台。本 fork 新增实验性的 RE Mode，用于二进制 triage、受保护软件分析、行为关联，以及可选的 Ghidra MCP-aware 工作流。

[官网](https://deepseek-gui.com) | [下载](https://deepseek-gui.com)

本 fork 保留原始 DeepSeek-GUI 桌面工作台，并新增专用 **Reverse Engineering Mode**。RE Mode 是启发式的二进制分析模式：它输出带置信度的候选发现，保留证据，把大体量原始输出写入磁盘，并帮助排序高价值逆向目标；它不会宣称自动完成完整脱壳、反虚拟化或语义恢复。

## 本 fork 新增内容

- **Reverse Engineering Mode**：独立的 Kun 模式和 GUI 入口。
- 支持 PE / ELF / Mach-O / APK 等文件的 binary triage。
- 静态分析 helper：哈希、字符串、熵、导入、导出、节区和格式检测。
- IOC 提取、能力评分、加壳指标和混淆指标。
- 受保护 / 混淆二进制分析，包括 VMProtect / Themida-like 虚拟化指标。
- VM dispatcher 和 VM handler 候选检测。
- decode/decrypt loop 候选与 dynamic API resolution 指标。
- 行为/证据图，用于关联 blob、decoder、decoded artifact、consumer 和 capability。
- 高价值逆向目标排序，并给出 confidence-rated reasons。
- `.re-mode/` 持久化工作区产物，适合长时间分析会话。
- 可选 Ghidra MCP-aware 函数分析；不可用时会回退到本地 helper 和已保存产物。

| 功能 | 说明 |
| --- | --- |
| **RE Mode** | 独立 Reverse Engineering 模式，包含专用 prompt、工具面、UI 入口和输出格式。 |
| **Binary Triage** | 检测格式、计算哈希、汇总节区、导入导出、字符串、熵、IOC 和能力线索。 |
| **Protected Binary Analysis** | 基于启发式的候选检测：加壳、虚拟化、decode/decrypt loop、动态 API 解析和可疑内存权限。 |
| **Behavior Graph** | 保存 high-entropy blob -> decoder -> decoded artifact -> consumer -> capability 等关系。 |
| **Ghidra MCP-aware Workflow** | 连接 Ghidra MCP 时，可读取反编译、xref、调用图、符号、注释和重命名能力。 |
| **Persistent RE Workspace** | 在 `.re-mode/` 下保存分析状态、IOC、符号、笔记、函数摘要、原始输出和报告。 |
| **Report Generation** | 根据已保存状态和分析发现生成 Markdown 逆向报告。 |
...
RE Mode 会把高体量原始输出留在磁盘上，并在工作区写入可复用状态：

- `.re-mode/analysis.json`：当前样本 metadata、哈希、格式、节区、导入导出、熵、能力线索和摘要。
- `.re-mode/iocs.json`：URL、domain、IP、email、注册表键、路径、命令、encoded blob、crypto constants 和 suspicious APIs。
- `.re-mode/symbols.json`：函数重命名、变量重命名、标签、符号备注和置信度。
- `.re-mode/behavior-graph.json`：关联函数、节区、blob、导入、能力、decoded artifact 和 stage 的证据图。
- `.re-mode/report.md`：生成的逆向分析报告。
- `.re-mode/raw/`：本地工具和外部工具的原始输出。
- `.re-mode/functions/`：标准化函数摘要，包括可用时来自 Ghidra 的上下文。

## 可选 Ghidra MCP 工作流

RE Mode 是 **Ghidra MCP-aware** 的。如果连接了 Ghidra MCP server，RE Mode 可以在后端支持时读取反编译、反汇编、xref、调用图、符号、函数摘要、重命名和注释。如果没有 Ghidra MCP，RE Mode 仍然可以使用本地 helper、通用 MCP discovery、已保存的 `.re-mode/` 产物和用户提供的片段继续工作。

详细文档：[`docs/re-mode.zh-CN.md`](docs/re-mode.zh-CN.md)。英文文档：[`docs/re-mode.md`](docs/re-mode.md)。

## Upstream

本 fork 基于原始项目：<https://github.com/XingYu-Zhong/DeepSeek-GUI>。原项目 credit 保留如下。
...
详细文档：[`docs/re-mode.zh-CN.md`](docs/re-mode.zh-CN.md)。
...
- 如果改动影响使用方式，请同步更新 `README.md` 和 `README.zh-CN.md`。
...
| [docs/re-mode.zh-CN.md](docs/re-mode.zh-CN.md) | Reverse Engineering 模式：二进制 triage、`.re-mode/` 产物、受保护二进制分析、证据图和 Ghidra MCP-aware 工作流 |
